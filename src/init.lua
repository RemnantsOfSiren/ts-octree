...
-- @classmod Octree
local OctreeRegionUtils = require(script.modules.OctreeRegionUtils);
local OctreeNode = require(script.modules.OctreeNode);
...
function Octree.new(maxSize)
    if not maxSize then
        maxSize = 512;
    end
	local self = setmetatable({}, Octree)

	self._maxRegionSize = { maxSize, maxSize, maxSize } -- these should all be the same number
	self._maxDepth = 4
